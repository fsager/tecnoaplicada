	...
	create table "APP"."PERSONA_EXAMEN"(
       ...
       "PEXA_OBS" VARCHAR(2000),
       "PEXA_ADJ" BLOB,
       "PEXA_RESULTADO_MEDICO" VARCHAR(100),
       "PEXA_NOMBRE_ADJUNTO" VARCHAR(200),
       "PEXA_TIPO_EXAMEN" VARCHAR(100),--Categoria del exámen.
       "PEXA_CANTIDAD_PREGUNTAS" NUMERIC(6),
       "PEXA_TIEMPO" NUMERIC(6),--en minutos
	   "PEXA_TIEMPO_UTILIZADO" NUMERIC(6),--en minutos
    ...
    --Exámes multiple choices
    --drop table "APP"."PREGUNTA";
    create table "APP"."PREGUNTA"(
        "ID" BIGINT not null generated  BY DEFAULT as identity,
        "PRE_DETALLE" VARCHAR(1000) not null,
        "PRE_CAT" VARCHAR(200) not null,--DOMINIO
        "PRE_VALORACION" BIGINT not null,
        "PRE_ORDEN" BIGINT not null,
        "PRE_IMAGEN" BLOB,       
        constraint "SQL100316234106610" primary key ("ID")
    );
    --drop table "APP"."RESPUESTA";
    create table "APP"."RESPUESTA"(
        "ID" BIGINT not null generated  BY DEFAULT as identity,
        "RES_DETALLE" VARCHAR(1000) not null,--DOMINIO
        "RES_CORRECTA" VARCHAR(1) not null,--DOMINIO S y N
		"PRE_ID" BIGINT not null,       
        constraint "SQL100315234106610" primary key ("ID")
    );
    
    --drop table "APP"."DETALLE_EXAMEN_MULTIPLE_CHOICE";
    create table "APP"."DETALLE_EXAMEN_MULTIPLE_CHOICE"(
        "ID" BIGINT not null generated  BY DEFAULT as identity,
        "PEXA_ID" BIGINT not null,
        "PRE_DETALLE" VARCHAR(1000) not null,
        "RES_DETALLE" VARCHAR(1000) not null,--DOMINIO
        ...
        constraint "SQL100315234116610" primary key ("ID")
    );
	--Exámes multiple choices
    

	alter table "APP"."RESPUESTA" add constraint "FK_00022" foreign key ("PRE_ID") references "APP"."PREGUNTA"("ID");
	
	alter table "APP"."DETALLE_EXAMEN_MULTIPLE_CHOICE" add constraint "FK_00032" foreign key ("PEXA_ID") references "APP"."PERSONA_EXAMEN"("PEXA_ID");
	alter table "APP"."DETALLE_EXAMEN_MULTIPLE_CHOICE" add constraint "FK_00033" foreign key ("PRE_ID") references "APP"."PREGUNTA"("ID");
	alter table "APP"."DETALLE_EXAMEN_MULTIPLE_CHOICE" add constraint "FK_00034" foreign key ("RES_ID") references "APP"."RESPUESTA"("ID");
