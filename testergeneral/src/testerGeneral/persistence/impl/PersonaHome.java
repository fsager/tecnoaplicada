...
public class PersonaHome extends DAOObject implements PersonaDao {
	...
	public void insert(Persona transientInstance) throws Exception {
		...
		try {
			...
			Util.insertAudit(Util.ACTION_INSERT, (Object) transientInstance,
					(Object) transientInstance.getPerNumeroDoc() + "-"
							+ transientInstance.getPerNombreCompleto());
	...
	public void delete(Persona persistentInstance) throws Exception {
		...
		try {
			...
			Util.insertAudit(Util.ACTION_DELETE, (Object) persistentInstance,
					(Object) persistentInstance.getPerNumeroDoc() + "-"
							+ persistentInstance.getPerNombreCompleto());
	...
	public Persona get(Long p_Id) throws Exception {
		...
		try {
			Persona instance = (Persona) getHibernateTemplate().get(
					Persona.class, p_Id);
			if (instance == null) {
	...
	public List getAll(Persona p_example) throws Exception {
		...
		try {
			...
			if (p_example.getPerEstudios() != null) {

				if (p_example.getPerEstudios().equalsIgnoreCase("SI")) {
					p_example.setPerEstudios(null);
					cri.add(Restrictions.isNotNull("perEstudios"));

				} else {
					if (p_example.getPerEstudios().equalsIgnoreCase("NO")) {
						p_example.setPerEstudios(null);
	...
	public List getAll(Persona p_example, Date desde, Date hasta)
			throws Exception {
		log.debug("finding Persona instance by example");
		try {
			Criteria cri = getSession().createCriteria(Persona.class);
